...
function toCloud(){Parse.User.current().save(null,{success:function(){throwMessage("All of your tabs have been sent to the cloud","chappy")}})}
...
for(var loop6=0;loop6<=10;loop6++){//row
    for(var loop7=0;loop7<=5;loop7++){//cell
        ...
        if(x.scheds[num][loop6][loop7]){
            ...
            if(isLab.indexOf(short)>=0){div.title="Lab"}
