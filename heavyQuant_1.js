...
var isLab=["Chem 11a","Chem 11b","Chem 29a","Chem 29b","Bio 18b","Phys 18a","Phys 18b","Phys 19a","Phys 19b"];

//AUTO
$(document).ready(function(){
    if(Parse.User.current()){
        startBranPlan(Parse.User.current());
    }else{
        slideIn();
    }
});
...
function startBranPlan(user){
    document.addEventListener("keydown",function(event){if(event.keyCode==32){findShift()}});
    currentUserDisp.textContent="You are currently logged in as "+Parse.User.current().get("username")+".";    
